...
public class MicroControllerGetTest {
    ...
    void setup() {
        ...
        when(microControllerService.getMicroControllerDetail(any())).thenReturn(generateMicroController());
    }
    ...
    class GetTest {
        ...
        void リクエストパラメータのマイコンUUIDがnullの場合400を返すこと() throws Exception {
            ...
            mockMvcPerform(DummyData.MICROCONTROLLER_GET_PATH).andExpect(status().isBadRequest());
        ...
        void リクエストパラメータのマイコンUUIDが空の場合400を返すこと() throws Exception {
            ...
            mockMvc.perform(MockMvcRequestBuilders.get(DummyData.MICROCONTROLLER_GET_PATH).param("microControllerUuid", "")).andExpect(status().isBadRequest());
        ...
        void リクエストパラメータのマイコンUUIDが不適切な値の場合400を返すこと() throws Exception {
            ...
            mockMvc.perform(MockMvcRequestBuilders.get(DummyData.MICROCONTROLLER_GET_PATH).param("microControllerUuid", DummyData.INVALID_UUID)).andExpect(status().isBadRequest());
        ...
        void リクエストパラメータのマイコンUUIDが適切な値の場合200を返すこと() throws Exception {
            ...
            mockMvc.perform(MockMvcRequestBuilders.get(DummyData.MICROCONTROLLER_GET_PATH).param("microControllerUuid", DummyData.VALID_UUID)).andExpect(status().isOk());
