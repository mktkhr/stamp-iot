...
class MicroControllerNoSessionGetTest {
	@Autowired
	...
	private fun mockMvcPerform(): ResultActions {
		return mockMvc.perform(
			MockMvcRequestBuilders.get(DummyData.MICROCONTROLLER_NO_SESSION_GET_PATH)
	...
	fun setup() {
		...
		whenever(microControllerService.getMicroControllerDetailWithMacAddress(any()))
	...
	internal inner class GetTest {
		@Test
		@Throws(Exception::class)
		fun リクエストパラメータのMACアドレスがnullの場合400を返すこと() {
			mockMvcPerform().andExpect(
		...
		@Test
		@Throws(Exception::class)
		fun リクエストパラメータのMACアドレスが空の場合400を返すこと() {
			mockMvc.perform(
				MockMvcRequestBuilders.get(DummyData.MICROCONTROLLER_NO_SESSION_GET_PATH).param("macAddress", "")
			).andExpect(MockMvcResultMatchers.status().isBadRequest())
		}

		@Test
		@Throws(Exception::class)
		fun リクエストパラメータのMACアドレスが不適切な値の場合400を返すこと() {
			mockMvc.perform(
				MockMvcRequestBuilders.get(DummyData.MICROCONTROLLER_NO_SESSION_GET_PATH)
					.param("macAddress", DummyData.INVALID_MAC_ADDRESS)
			).andExpect(MockMvcResultMatchers.status().isBadRequest())
		}

		@Test
		@Throws(Exception::class)
		fun リクエストパラメータのMACアドレスが適切な値の場合200を返すこと() {
			mockMvc.perform(
				MockMvcRequestBuilders.get(DummyData.MICROCONTROLLER_NO_SESSION_GET_PATH)
					.param("macAddress", DummyData.VALID_MAC_ADDRESS)
